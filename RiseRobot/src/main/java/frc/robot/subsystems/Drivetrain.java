...
public class Drivetrain extends WaltSubsystem {

  private final AHRS ahrs = new AHRS(SPI.Port.kMXP);
  private final CANSparkMax rightWheelsMaster = new CANSparkMax(DRIVE_RIGHT_MASTER_ID,
      CANSparkMax.MotorType.kBrushless);
  private final CANSparkMax rightWheelsSlave = new CANSparkMax(DRIVE_RIGHT_SLAVE_ID,
      CANSparkMax.MotorType.kBrushless);
  private final CANSparkMax leftWheelsMaster = new CANSparkMax(DRIVE_LEFT_MASTER_ID,
      CANSparkMax.MotorType.kBrushless);
  private final CANSparkMax leftWheelsSlave = new CANSparkMax(DRIVE_LEFT_SLAVE_ID,
      CANSparkMax.MotorType.kBrushless);

  private DifferentialDriveKinematics driveKinematics = new DifferentialDriveKinematics(
      currentRobot.getTrackWidth());
  ...
  public void setVelocities(double leftVelocity, double leftFeedForward, double rightVelocity,
      double rightFeedForward, int sparkMaxPIDSlot) {
    leftWheelsMaster.getPIDController()
        .setReference(leftVelocity, ControlType.kVelocity, sparkMaxPIDSlot, leftFeedForward);
    rightWheelsMaster.getPIDController()
        .setReference(rightVelocity, ControlType.kVelocity, sparkMaxPIDSlot, rightFeedForward);
  }
  ...
  private void updateRobotPose() {
    robotPose = driveOdometry.update(getHeading(), leftWheelsMaster.getEncoder().getPosition(),
        rightWheelsMaster.getEncoder().getPosition());
  }
  ...
  public Supplier<ArrayList<String>> getPitCheckFunction() {
    return () -> {
      ...
      if (leftWheelsMaster.getLastError() != CANError.kOk) {
        failures.add("Left Master Controller");
      }
      if (leftWheelsSlave.getLastError() != CANError.kOk) {
        failures.add("Left Slave Controller");
      }
      if (rightWheelsMaster.getLastError() != CANError.kOk) {
        failures.add("Right Master Controller");
      }
      if (rightWheelsSlave.getLastError() != CANError.kOk) {
        failures.add("Right Slave Controller");
      }
